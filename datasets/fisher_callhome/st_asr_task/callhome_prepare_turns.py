...
- We append the TURN token between sentences!
...
def prepare_turns_only_callhome_spanish(
    data_folder: str,
    save_folder: str,
    ...
    >>> save_foler = 'data'
    >>> prepare_turns_only_callhome_spanish(data_folder, save_folder)
    """
    ...
    make_data_splits(f"{corpus_path}/mapping")

    for dataset in datasets:
        sub_folder_path = f"callhome-{dataset}-{int(max_utterance_allowed)}s"
        ...
        # filter out empty transcription/translation
        concated_data = list(
            filter(lambda data: 0 < len(data.transcription), concated_data)
        )
...
def extract_transcription(transcription_path: str) -> List[TDF]:
    ...
    with open(transcription_path, "r", encoding="ISO-8859-1") as transcription_file:
        ...
        for transcription in transcriptions:
            ...
            if "A" in channel or "0" in channel:
                channel = 0
            elif "B" in channel or "1" in channel:
                channel = 1
            else:
                continue
...
def concate_transcriptions_by_max_utterance(
    ...
    def join_data_object(data_in1, data_in2):
        ...
        assert data_in1.uid.split("-")[0] == data_in2.uid.split("-")[0]
        ...
        if channel_turn_list[-1] != data_in2.channel:
            ...
            dur_turn_list.append(data_in2.duration)
        ...
        new_data = Data(
            uid=uttrance_id,
            wav=f"{subset_folder_name}/{uid}.wav 0 {start_d1} {end_d2}",
    ...
    for sample in extracted_transcriptions:
        ...
        # keep the spaces
        sample.wav = sample.wav.replace("wav 1 ", "wav 0 ")
...
def concate_transcriptions_by_mapping_file(
    ...
    with open(mapping_file_path, "r", encoding="utf-8") as callhome_mapping_file:
        ...
        for callhome_mapping_line in callhome_mapping:
            ...
            uttrance_id = f"{uid}-{channel}-{start:06d}-{end:06d}"
...
if __name__ == "__main__":
    ...
    prepare_turns_only_callhome_spanish(data_folder, save_folder, device=device)
